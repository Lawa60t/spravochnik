...
const DETAIL_FORBIDDEN = [
  ...
  [/(?<![а-яё])[а-яё]+(циллин|мицин|цефалоспорин|флоксацин|прил|сартан|олол|статин|празол|профен|триптан)[а-яё]*/i, "название средства по суффиксу"],
  [/(?<![а-яё])(парацетамол|аспирин|анальгин|ибупрофен|нурофен|но-?шпа|дротаверин|нимесулид|диклофенак|кетопрофен|амоксициллин|азитромицин|омепразол|лоперамид|смекта|энтеросгель|валидол|нитроглицерин|корвалол|валокордин|супрастин|цетиризин|лоратадин|преднизолон|дексаметазон|инсулин|метформин|фуросемид)(?![а-яё])/i, "название средства"]
];
conditions.forEach(c => {

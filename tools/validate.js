...
/* \b в JavaScript не знает кириллицы, поэтому после «мг» — просмотр «дальше не буква». */
const TREATMENT = /(\d+\s?мг(?![а-яё])|дозиров|принимать по|таблетк[аи] .*раз в день|курс антибиотик|назначают препарат)/i;
...
/* Подробная статья (detail) и красная плашка (emergency).

   В detail разрешены общие слова о лечении: «поддаётся лечению», «лечат
   амбулаторно или в стационаре», «устраняется операцией», «наблюдают»,
   «назначения делает врач». Запрещены и здесь: названия средств, дозы,
   схемы, длительность курса и любые «попробуйте / принимайте / можно самому».
   Короткие поля проверяет TREATMENT выше, как и раньше.

   Список не является исчерпывающим и быть им не может: он ловит оплошность,
   а не намеренное нарушение. */
const DETAIL_FORBIDDEN = [
  /* \b в JavaScript знает только латиницу: после кириллицы границы слова нет.
     Поэтому вместо \b — просмотр «дальше не буква». */
  [/\d+\s?(мг|мл|мкг|г)(?![а-яё])/i, "доза"],
  [/дозиров/i, "дозировка"],
  [/\d+\s*раз\w*\s+в\s+(день|сутки|неделю|месяц)/i, "схема приёма"],
  [/(курс\w*|в течение|на протяжении)\s+(\d+|нескольких|двух|трёх|пяти|семи|десяти)\s*(дн|недел|месяц)/i, "длительность курса"],
  [/принимайте|принимать по|попробуйте|попробовать самостоятельно/i, "указание читателю"],
  [/можно\s+(самому|самостоятельно|дома)\s+(принять|выпить|снять|лечить|вылечить)/i, "самолечение"],
  /* Названия средств по суффиксу международного названия.
     Суффикс обязан стоять НА КОНЦЕ слова: без этого «кололо», «заколол» и
     «неприлично» ловились как названия лекарств и роняли сборку. */
  [new RegExp("(?<![а-яё])[а-яё]{2,}(" + [
    "циллин", "мицин", "флоксацин", "цефалоспорин", "цефтриаксон", "цефиксим",
    "прил", "сартан", "дипин", "тиазид", "пролол", "тенолол", "нолол", "дилол", "волол",
    "статин", "фибрат", "празол", "тидин", "профен", "коксиб", "триптан", "сетрон",
    "маб", "ниб", "коназол", "циклин", "парин", "ксабан", "гатран", "метазон", "афил",
    "глифлозин", "глиптин", "тропий", "терол", "сонид", "каин", "золам", "зепам"
  ].join("|") + ")(?![а-яё])", "i"), "название средства по суффиксу"],

  /* «вир» отдельно и с более длинной основой: иначе совпадает со словом «клавир». */
  [/(?<![а-яё])[а-яё]{4,}вир(?![а-яё])/i, "название средства по суффиксу"],

  /* Все падежи «инсулина», «инсулином», но не «инсулинорезистентность»:
     последнее — термин, а не название средства, и оно употреблено в статье pcos. */
  [/(?<![а-яё])инсулин(?!орезистент)[а-яё]*/i, "название средства"],

  /* Явный список — то, у чего узнаваемого суффикса нет.
     ГОРМОНЫ ЗДЕСЬ ТОЛЬКО В ЛЕКАРСТВЕННОЙ ФОРМЕ (левотироксин, преднизолон,
     гидрокортизон). Природные названия — тироксин, кортизол, эстроген, прогестерон,
     тестостерон, пролактин — НЕ запрещены и запрещены быть не могут: замороженная база
     сама употребляет «кортизол», «эстрогенов», «тестостерон» в коротких полях, а СПКЯ и
     климакс без них не описать. Единственное исключение — «инсулин»: слово обозначает
     и гормон, и средство, поэтому в подробных статьях оно остаётся запрещённым, а в
     коротких полях базы сохраняется (это правило их не проверяет). */
  [new RegExp("(?<![а-яё])(" + [
    "парацетамол", "аспирин", "анальгин", "ибупрофен", "нурофен", "но-?шпа", "дротаверин",
    "нимесулид", "диклофенак", "кетопрофен", "амоксициллин", "азитромицин", "омепразол",
    "лоперамид", "смекта", "энтеросгель", "валидол", "нитроглицерин", "корвалол",
    "валокордин", "супрастин", "цетиризин", "лоратадин", "преднизолон", "дексаметазон",
    "метформин", "фуросемид",
    "сальбутамол", "аллопуринол", "колхицин", "варфарин", "метотрексат", "тамсулозин",
    "финастерид", "клопидогрел", "морфин", "трамадол", "амитриптилин", "циклоспорин",
    "изотретиноин", "тербинафин", "гидроксихлорохин", "левотироксин", "гидрокортизон",
    "азатиоприн", "сертралин", "флуоксетин", "лозартан", "будесонид", "формотерол",
    "ацетилсалицилов", "ципрофлоксацин", "цефалексин", "де-?нол", "мезим", "фестал",
    "регидрон", "ксилометазолин", "оксиметазолин"
  ].join("|") + ")(?![а-яё])", "i"), "название средства"]
];
conditions.forEach(c => {
  if (c.emergency !== undefined && typeof c.emergency !== "boolean") E(`${c.id}: emergency должен быть true/false`);
  if (c.detail === undefined) return;
  if (!Array.isArray(c.detail) || !c.detail.length) { E(`${c.id}: detail должен быть непустым массивом`); return; }
  c.detail.forEach((b, i) => {
    if (!b || typeof b.h !== "string" || !b.h.trim() || typeof b.p !== "string" || !b.p.trim()) {
      E(`${c.id}: detail[${i}] — нужны непустые строки h и p`); return;
    }
    const extra = Object.keys(b).filter(k => k !== "h" && k !== "p");
    if (extra.length) E(`${c.id}: detail[${i}] — лишние поля ${extra.join(", ")}`);
    const txt = b.h + " " + b.p;
    DETAIL_FORBIDDEN.forEach(([re, why]) => {
      const m = txt.match(re);
      if (m) E(`${c.id}: detail[${i}] «${b.h}» — ${why}: «${m[0]}»`);
    });
  });
});

/* Пол в статье должен быть проставлен там, где его требует само название.
   Найдено при проверке: «Бактериальный вагиноз» без sexOnly показывался
   мужчине. Ловим по жёстким половым маркерам в названии. Список исключений
   явный: молочная железа не в нём намеренно — мужчины болеют (гинекомастия,
   рак молочной железы без ограничения по полу), это оговорено в CLAUDE.md,
   и слова «молочн» в маркерах нет. Здесь только статьи, у которых маркер
   в названии есть, а пол по существу не ограничен. */
const SEX_MARKERS = /(влагалищ|вульв|матк|яичник|менструа|предстательн|яичк|мошонк|эректильн)/i;
const SEX_MARKER_EXCEPTIONS = new Set([
  /* пока пусто: каждое исключение — с причиной в комментарии рядом */
]);
conditions.forEach(c => {
  if (c.sexOnly || SEX_MARKER_EXCEPTIONS.has(c.id)) return;
  const m = c.name.match(SEX_MARKERS);
  if (m) W(`${c.id}: в названии половой маркер «${m[0]}», а sexOnly не проставлен — «${c.name}»`);
});

/* Возрастная жёсткая граница без самой границы бессмысленна. */
conditions.forEach(c => {
  if (c.ageStrict && c.ageMin === undefined && c.ageMax === undefined)
    E(`${c.id}: ageStrict без ageMin/ageMax`);
});
...
/* --- слово «симптом» в данных ---
   Линт формулировок видит только собранную страницу и только само слово,
   без окружения: разрешить его там можно лишь целиком на весь адрес. Здесь
   окружение есть, поэтому точная охрана стоит именно тут.

   Разрешена ровно одна строка. Это поисковый синоним раздела «Болезни,
   которые не дают о себе знать»: человек так и набирает, и убрать его значит
   потерять находимость. Голосом сайта он не является — как и названия
   средств в вариантах ответа questions.json, которые человек опознаёт среди
   того, что уже принимает. Решение владельца 09.09.2026, подтверждено
   10.09.2026.

   «Бессимптомно» и «бессимптомный» просмотр (?<![а-яё]) пропускает намеренно:
   в описании аневризмы или фибрилляции это единственное точное слово.

   Появится второе такое место — падает эта проверка, даже если линт сборки
   промолчит, потому что страница уже внесена в исключения. */
const SIMPTOM = /(?<![а-яё])симптом/i;
const SIMPTOM_OK = ["болезни без симптомов"];
(function () {
  const seen = [];
  const walk = (node, where) => {
    if (typeof node === "string") { if (SIMPTOM.test(node)) seen.push([where, node]); return; }
    if (Array.isArray(node)) return node.forEach((v, i) => walk(v, `${where}[${i}]`));
    if (node && typeof node === "object")
      return Object.entries(node).forEach(([k, v]) => walk(v, `${where}.${k}`));
  };
  files.filter(f => f.endsWith(".json")).forEach(f => walk(read(f), f));
  seen.forEach(([where, text]) => {
    if (!SIMPTOM_OK.includes(text))
      E(`«симптом» в данных — ${where}: «${text}». Разрешено только как поисковый синоним gen.silent`);
  });
  SIMPTOM_OK.forEach(t => {
    if (!seen.some(([, text]) => text === t))
      W(`разрешённая строка «${t}» из данных исчезла — исключение в lint-allow.json стало лишним`);
  });
})();

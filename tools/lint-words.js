...
const HARD = [
  ...
  [/совпадени[а-яё]*\s+\d/i, "числовое совпадение"],
  [/точность\s+\d/i, "заявление о точности"],
  [/(?<![а-яё])у\s+вас(?![а-яё])/i, "«у вас» — вывод говорит о болезнях, а не о читателе"],
  [/ваш\w*\s+диагноз/i, "«ваш диагноз»"],
  [/вероятн[а-яё]*\s+диагноз/i, "«вероятный диагноз»"],
  [/онлайн[-\s]?диагностик/i, "«онлайн-диагностика»"],
  [/определ[а-яё]+\s+болезнь/i, "«определить болезнь»"],
  [/пройдите\s+тест/i, "«пройдите тест»"],
  [/начать\s+обследование/i, "«начать обследование»"],
  [/риск[а-яё]*\s+на\s+себя/i, "перенос рисков на читателя"],
  [/принимаете\s+риск/i, "перенос рисков на читателя"],
  [/не\s+нес[её]м\s+ответственност/i, "«не несём ответственности»"],
  /* Лечение: запрещены средства, дозы, схемы и длительность курса — везде,
     включая подробные статьи (detail). Общие слова о лечении («поддаётся
     лечению», «лечат амбулаторно», «устраняется операцией», «наблюдают»,
     «назначения делает врач») этим списком не ловятся намеренно: в detail
     они разрешены. Что разрешено и что нет — в CLAUDE.md. */
  /* \b в JavaScript не знает кириллицы: после «мг» граница слова не находится.
     Поэтому вместо \b — просмотр «дальше не буква». То же ограничение
     касается и \w: на кириллице он не совпадает ни с одной буквой, поэтому
     правило с ним молчит всегда. Оба заменены кириллическими просмотрами
     и классом [а-яё] — иначе проверка выглядит рабочей, а не работает. */
  [/\d+\s?мг(?![а-яё])/i, "доза"],
  [/дозиров/i, "дозировка"],
  [/принимать\s+по(?![а-яё])/i, "схема приёма"],
  [/курс\s+антибиотик/i, "схема лечения"],
  [/назначают\s+препарат/i, "назначение препарата"],
  [/(?<![а-яё])принимайте(?![а-яё])/i, "указание читателю принимать"],
  /* «Попробуйте другое слово» в поиске законно; ловим только «попробуйте»
     в смысле лечения. В подробных статьях (detail) валидатор запрещает
     слово целиком, без оговорок. */
  [/(?<![а-яё])попробуйте\s+(принять|выпить|пить|пропить|приложить|подышать|сам\w*|дома|средств\w*|препарат\w*|лекарств\w*|таблетк\w*)/i, "«попробуйте …» — совет по лечению"],
  [/можно\s+(самому|самостоятельно)\s+(принять|выпить|лечить|вылечить)/i, "самолечение"],
  /* Четыре слова, объявленные в CLAUDE.md запрещёнными на экранах, стояли
     на мягком уровне — то есть проверялись только в title и description.
     Замер 10.09.2026 показал, чем это кончилось: «болезни без симптомов»
     печаталось на трёх собранных страницах при зелёной сборке. Правила
     подняты на жёсткий уровень, то есть на весь текст страницы.
     «Бессимптомно» просмотр (?<![а-яё]) пропускает намеренно: в описании
     болезни это единственное точное слово, и заменять его нечем. */
  [/ваши\s+симптом/i, "«ваши симптомы» → «выберите описание, которое ближе»"],
  [/(?<![а-яё])симптом/i, "«симптом» в тексте сайта → «ощущение», «описание»"],
  [/(?<![а-яё])анкет/i, "«анкета» → «уточнение»"],
  [/(?<![а-яё])опрос(?![а-яё])/i, "«опрос» → «уточнение»"],
  [/(?<![а-яё])диагностик/i, "«диагностика» в тексте сайта"]
];

const SOFT = [
  [/возможн[а-яё]*\s+причин/i, "«возможные причины» → «состояния, при которых так бывает»"],
...
/* Два исключения из списка выше — надстройки, которые стоят на всех
   страницах. Правило при этом не ослаблено — оно осталось тем же, каким было
   задумано: страница обязана быть полной без единого скрипта.

   Поиск в шапке: стоит везде, потому что иначе его не находят — указатель
   существует, но до него надо додуматься дойти. Поле создаётся скриптом
   и без него не появляется вовсе; на его месте остаётся обычная ссылка
   на указатель, а весь текст страницы уже отрисован.

   Кнопка «Наверх»: стоит везде, потому что длинная страница длинна на любом
   адресе. В разметке она скрыта атрибутом hidden, показывает её только
   скрипт; без него страница выглядит так, будто кнопки не существует,
   а прокрутка работает сама. Ничего не хранит, никуда не ходит.
...
const ON_EVERY_PAGE = [/^\/poisk\.[0-9a-f]{8}\.js$/, /^\/naverkh\.[0-9a-f]{8}\.js$/];
const isEverywhere = src => ON_EVERY_PAGE.some(re => re.test(src));
...
function run() {
  ...
  files.forEach(file => {
    ...
    scripts.forEach(tag => {
      ...
      if (!isEverywhere(src) && !isScripted(rel))

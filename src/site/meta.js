...
module.exports = {
  ...
  zones: nZones => ({
    title: `Области тела — оглавление справочника`,
    description: clamp(
  ...
  zone: (zone, count) => ({
    title: `${zone.name} — область справочника`,
    description: clamp(
      `${zone.name}: ${count} ${plural(count, "раздел", "раздела", "разделов")} справочника по участкам. Что это, у кого встречается чаще, к какому врачу.`
    )
  }),

  zhaloby: title => ({
    title: `${title} · Справочник о теле`,
    description: clamp(
      "Тошнота, головокружение, кровь в стуле и другие жалобы, которые нельзя показать на модели тела. Отдельный вход в разделы справочника."
    )
  }),

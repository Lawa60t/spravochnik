...
function present(syndromeId, input) {
  ...
  /* Порядок блоков — порядок чтения. Справочник не ставит диагноз, поэтому
     выдача начинается с частого, а не с редкого и страшного: «редко, но важно»
     идёт последним, как и на статической странице раздела, где он стоит
     последним по той же причине. Последним — не значит спрятанным: на сайте
     он развёрнут, в отличие от страницы раздела. Тревоги (alarms) сюда
     не входят и показываются над всем остальным — это отдельный слой. */
  return {
    alarms,
    blocks: {
...
/* Насколько надбавка слабеет с каждым недостающим ответом. Делить на число
   недостающих нельзя: правилу из трёх условий доставалось 50 против 100
   за смену первого места, и оно проигрывало всегда. Именно так терялись
   ишемия кишечника (мягкий живот + не находит места + внезапно)
   и лейкоз (плотные узлы + больше двух недель). */
const RULE_STEP = 25;
...
function ruleBonus(qid, answers, rules) {
  ...
  rules.forEach(r => {
    ...
    best = Math.max(best, REDFLAG_BONUS - (Math.max(missing, 1) - 1) * RULE_STEP);
  });
  return best;
}
